"""Student Feedback Form Streamlit application.

This module implements a Streamlit-based web form for collecting student
feedback on academic programs at Texas A&M University. The form captures
ratings on course satisfaction, learning outcomes, support services, and
open-ended feedback. Submissions are stored as JSON files in an AWS S3 bucket.
"""
...
# Custom theme applying Texas A&M branding colors and fonts
st.markdown("""
<style>
...
# Set up S3 client using environment variables for credential configuration
s3 = boto3.client('s3',
...
# S3 bucket name for storing feedback submissions
S3_BUCKET_NAME = "awsbin-amazonq-assets"

# Header section with university logo and title
...
# Main feedback form - collects structured and open-ended feedback
with st.form("feedback_form"):
    # Generate an anonymized student ID to protect respondent identity
    ...
    # Likert scale ratings for various program dimensions
    ...
    # Multi-select for identifying specific improvement areas
    ...
    # Expandable section for additional qualitative feedback
...
# Handle form submission with validation and S3 upload
if submitted:
    if not program_name:
        ...
        # Assemble all feedback data into a structured dictionary
        feedback_data = {
        ...
        # Convert data to JSON string for S3 storage
        ...
        try:
            # Show a visual progress bar during upload
            progress_bar = st.progress(0)
            for i in range(100):
                time.sleep(0.01)
                progress_bar.progress(i + 1)

            # Upload the feedback JSON to S3
            s3.put_object(
...
# Sidebar with FAQ and privacy information
